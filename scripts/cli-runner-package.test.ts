...
it("keeps browser SDK calls and one-use permissions on the same connection", async () => {
  ...
  await writeFile(
    ...
    let publicReplies = 0;
    window.__HOST_API_PORT__.addEventListener('message', () => publicReplies++);
    ...
    await assert.rejects(Promise.resolve(client.permissions.requestRemotePermission(permission)), /message port closed unexpectedly/);
    await assert.rejects(fetch('http://127.0.0.1:${server.port}/closed'), /Network access is not allowed/);
    console.log(JSON.stringify({ sdkFailed, fetchFailed, publicReplies, status }));
