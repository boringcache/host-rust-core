...
- `LocalhostBridgeBootstrap` - JS snippet that publishes the WS bridge endpoint (`window.__truapi_localhost`) to the product page so it can dial back in, republishes a port when that socket dies, and defines the `window.__pauseConnections__` / `window.__resumeConnections__` hooks the host calls on app lifecycle.
