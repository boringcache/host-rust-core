...
Two steps around them are the host's. `establishPairing` submits its answer under the peer's own device statement allowance, so track that account as a renewal target first: `parsePairingDeeplink` reads the peer out of the deeplink before any notice goes out, and a pairing that then fails untracks it again. `disconnectPairedHost` submits the notice and nothing more, so ending a pairing also means cancelling that peer's `resumePairing` coroutine and untracking its renewal account; dropping the stored pairing alone leaves both running.
