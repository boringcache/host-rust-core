...
A relayed signature is outside this: it carries no caller identity and the signing role confirms each one with the user, so a paired Host relays a cross-product signature rather than serving it from an AutoSigning fast path the grant was never meant to open.
