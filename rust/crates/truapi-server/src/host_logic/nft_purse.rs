//! NFT purse key derivation: one sr25519 purse key per `pallet-scarcity`
...
//! ```
//!
//! Every junction is hard. sr25519 soft derivation is invertible from the
//! child side: a child secret together with the parent public key and the
//! path recovers the parent secret. A purse key that ever leaves the host must
//! therefore not be soft-derived from anything the host wants to keep. The
//! purse junction is the product id itself, so recovery from seed needs only
//! the product ids the host knows, and the wallet's own purse is the reserved
//! product [`WALLET_PURSE_PRODUCT_ID`].
...
/// Reserved product id of the wallet's own purse. Governance reserves product
/// ids of five characters or fewer, so no product can claim it.
pub const WALLET_PURSE_PRODUCT_ID: &str = "nfts.dot";

/// Leading junctions shared by every purse key.
const PURSE_JUNCTIONS: [&str; 2] = ["pps", "nft"];

/// Error deriving a purse key.
#[derive(Debug, PartialEq, Eq, derive_more::Display)]
pub enum NftPurseDerivationError {
...
pub fn normalize_purse_product_id(product_id: &str) -> Result<String, NftPurseDerivationError> {
    let normalized = normalize_product_identifier(product_id).map_err(|_| {
        NftPurseDerivationError::InvalidProductId {
            product_id: product_id.to_string(),
        }
    })?;
    if normalized.bytes().all(|byte| byte.is_ascii_digit()) {
        return Err(NftPurseDerivationError::NumericProductId {
            product_id: product_id.to_string(),
        });
    }
    Ok(normalized)
...
pub fn derive_purse_keypair(
    ...
    product_id: &str,
    index: u32,
) -> Result<Keypair, NftPurseDerivationError> {
    ...
    derive_sr25519_hard_path(entropy, &junctions).map_err(NftPurseDerivationError::Derivation)
}
...
pub fn derive_purse_public_key(
    ...
    product_id: &str,
    index: u32,
) -> Result<[u8; 32], NftPurseDerivationError> {
...
mod tests {
    ...
    fn invalid_purse_ids_are_rejected() {
        assert!(matches!(
            ...
            Err(NftPurseDerivationError::InvalidProductId { .. })
        ));
        assert!(matches!(
            derive_purse_keypair(&ENTROPY, "", 0),
            Err(NftPurseDerivationError::InvalidProductId { .. })
        ));
        // An all-digit id is refused whether or not it parses as a product
        // identifier: it would collide with the index junction space.
        assert!(derive_purse_keypair(&ENTROPY, "12345", 0).is_err());
    }
