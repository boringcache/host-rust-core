//! Durable NFT-purse records for one wallet: each purse's index counter, the
//! receive keys handed out but not yet seen holding an item, and the
//! write-ahead log of transfers broadcast but not yet settled.
//!
...
pub(crate) struct TransferLogEntry {
    ...
    pub from_index: u32,
    /// The holding key itself, kept so recovery needs no derivation.
...
pub(crate) struct TransferLog {
    ...
    pub entries: Vec<TransferLogEntry>,
}

/// Cache-fronted, guard-serialized purse store.
pub(crate) struct NftPurseStore {
    platform: Arc<dyn Platform>,
    cache: Mutex<HashMap<[u8; 32], PursesSnapshot>>,
    log_cache: Mutex<HashMap<[u8; 32], TransferLog>>,
...
impl NftPurseStore {
    ...
    ) -> Result<PursesSnapshot, StoreError> {
    ...
    pub(crate) async fn purse(
        ...
        root_public_key: [u8; 32],
        product_id: &str,
    ) -> Result<Option<PurseRecord>, StoreError> {
        Ok(self
    ...
    pub(crate) async fn allocate(
        ...
        purse.next_index = index
            .checked_add(1)
            .ok_or_else(|| StoreError::Storage("purse index space exhausted".into()))?;
    ...
    /// Record that a scan saw items at `occupied` in `product_id`'s purse, so
    /// allocation resumes above them after a restore from seed, and drop
    /// reservations for keys now seen occupied.
    ...
    pub(crate) async fn log_entries(
        &self,
        root_public_key: [u8; 32],
    ) -> Result<Vec<TransferLogEntry>, StoreError> {
        let _guard = self.storage_guard.lock().await;
        Ok(self.load_log_under_guard(root_public_key).await?.entries)
    }

    /// Record a transfer about to be broadcast; returns its id.
    pub(crate) async fn log_append(
        &self,
        root_public_key: [u8; 32],
        mut entry: TransferLogEntry,
    ) -> Result<u64, StoreError> {
        let _guard = self.storage_guard.lock().await;
        let mut log = self.load_log_under_guard(root_public_key).await?;
        entry.id = log.next_id;
        log.next_id = log
            .next_id
            .checked_add(1)
            .ok_or_else(|| StoreError::Storage("transfer log id space exhausted".into()))?;
        let id = entry.id;
        log.entries.push(entry);
        self.persist_log_under_guard(root_public_key, log).await?;
        Ok(id)
    }

    /// Drop a resolved transfer.
    pub(crate) async fn log_remove(
        &self,
        root_public_key: [u8; 32],
        id: u64,
    ) -> Result<(), StoreError> {
        ...
        log.entries.retain(|entry| entry.id != id);
    ...
    async fn load_log_under_guard(
        &self,
        root_public_key: [u8; 32],
    ) -> Result<TransferLog, StoreError> {
        if let Some(log) = self
            .log_cache
            .lock()
            .expect("purse transfer log cache mutex poisoned")
            .get(&root_public_key)
            .cloned()
        {
            return Ok(log);
        }
        let key = CoreStorageKey::NftPurseTransferLog { root_public_key };
        let log = match self
            .platform
            .read_core_storage(key.clone())
            .await
            .map_err(|err| StoreError::Storage(err.reason))?
        {
            Some(blob) => match decode_exact::<TransferLog>(&blob) {
                Ok(log) => log,
                Err(error) => {
            ...
            None => TransferLog::default(),
        };
        self.log_cache
            .lock()
            .expect("purse transfer log cache mutex poisoned")
            .insert(root_public_key, log.clone());
        Ok(log)
    }

    async fn persist_log_under_guard(
        &self,
        root_public_key: [u8; 32],
        log: TransferLog,
    ) -> Result<(), StoreError> {
        self.platform
            .write_core_storage(
                CoreStorageKey::NftPurseTransferLog { root_public_key },
                log.encode(),
            )
            .await
            .map_err(|err| StoreError::Storage(err.reason))?;
        self.log_cache
            .lock()
            .expect("purse transfer log cache mutex poisoned")
            .insert(root_public_key, log);
        Ok(())
    }

    fn cached(&self, root_public_key: [u8; 32]) -> Option<PursesSnapshot> {
        self.cache
            .lock()
            .expect("purse store cache mutex poisoned")
            .get(&root_public_key)
            .cloned()
    }
    ...
    async fn load_under_guard(
        ...
        let key = CoreStorageKey::NftPurses { root_public_key };
        let snapshot = match self
            ...
            .map_err(|err| StoreError::Storage(err.reason))?
        {
            Some(blob) => match decode_exact::<PursesSnapshot>(&blob) {
    ...
    async fn persist_under_guard(
        &self,
        root_public_key: [u8; 32],
        snapshot: PursesSnapshot,
    ) -> Result<(), StoreError> {
        self.platform
            .write_core_storage(
                CoreStorageKey::NftPurses { root_public_key },
                snapshot.encode(),
            )
            .await
            .map_err(|err| StoreError::Storage(err.reason))?;
...
fn purse_mut<'a>(snapshot: &'a mut PursesSnapshot, product_id: &str) -> &'a mut PurseRecord {
...
fn decode_exact<T: Decode>(blob: &[u8]) -> Result<T, StoreError> {
    let mut input = blob;
    let value = T::decode(&mut input).map_err(|error| StoreError::Corrupt(error.to_string()))?;
    if !input.is_empty() {
...
mod tests {
    ...
    fn allocation_is_monotonic_and_replays_by_caller_and_key() {
        futures::executor::block_on(async {
            let store = NftPurseStore::new(Arc::new(StubPlatform::default()));
            let a = store
    ...
    fn a_corrupt_slot_is_cleared_rather_than_trusted() {
        futures::executor::block_on(async {
            ...
            let key = CoreStorageKey::NftPurses {
            ...
            platform
                ...
                .await
                .unwrap();
            let store = NftPurseStore::new(platform.clone());
            assert!(matches!(
                store.snapshot(ROOT).await,
                Err(StoreError::Corrupt(_))
            ));
            assert!(platform.read_core_storage(key).await.unwrap().is_none());
            // The next read starts from an empty set of purses.
    ...
    fn the_transfer_log_appends_ids_and_forgets_resolved_entries() {
        futures::executor::block_on(async {
            let platform = Arc::new(StubPlatform::default());
            let store = NftPurseStore::new(platform.clone());
            let entry = |instance: u64| TransferLogEntry {
                id: 0,
            ...
            let a = store.log_append(ROOT, entry(34)).await.unwrap();
            let b = store.log_append(ROOT, entry(35)).await.unwrap();
            assert_eq!((a, b), (0, 1));
            store.log_remove(ROOT, a).await.unwrap();
            let entries = store.log_entries(ROOT).await.unwrap();
            assert_eq!(entries.len(), 1);
            assert_eq!(entries[0].instance, 35);
            // A fresh store reads the same log and keeps allocating ids forward.
            let again = NftPurseStore::new(platform);
            assert_eq!(again.log_append(ROOT, entry(36)).await.unwrap(), 2);
        });
    }
    ...
    fn snapshots_survive_a_fresh_store_over_the_same_platform() {
        futures::executor::block_on(async {
            let platform = Arc::new(StubPlatform::default());
            let first = NftPurseStore::new(platform.clone());
            first
                .allocate(ROOT, "seity.dot", "seity.dot", "pin")
                .await
                .unwrap();
            let second = NftPurseStore::new(platform);
