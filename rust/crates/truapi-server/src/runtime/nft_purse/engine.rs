//! The purse engine: per-product `pallet-scarcity` purses over the wallet's
//! root entropy, served by the signing host.
//!
//! Each product has one purse, derived at `//pps//nft//<product_id>//<index>`;
//! the wallet's own is the reserved `nfts.dot`. The chain is the authority on
//! what a purse holds; the engine scans `NftsByOwner`, hands out never-reused
//! receive keys, moves items, and remembers only what it allocated and what it
//! broadcast.

use std::collections::HashMap;
use std::sync::Arc;

use truapi::latest::{
    NftPurseError, NftPurseItem, NftPurseTransferStatus, NftPurseTransferability,
};
use truapi_platform::NftPurseTransferReview;

use super::chain::{self, ChainError, Nft, Transferability};
use super::store::{NftPurseStore, StoreError};
use super::transfer::{self, TransferError};
use crate::host_logic::nft_purse::{
    NftPurseDerivationError, derive_purse_public_key, normalize_purse_product_id,
};
use crate::runtime::services::RuntimeServices;
use crate::runtime::statement_allowance::rpc::RpcClient;
use crate::runtime::statement_allowance::{ChainClient, ChainContext, StatementAllowanceError};

/// Keys read per scan round trip.
const SCAN_BATCH: u32 = 10;
/// Hard ceiling on indices a scan will walk past the last occupied key.
pub(crate) const SCAN_MAX: u32 = 1_000;

/// Failure inside the purse engine.
#[derive(Debug, derive_more::Display)]
pub(crate) enum Error {
    /// The host serves no Asset Hub, so there is no Scarcity pallet to read.
    #[display("the host serves no Asset Hub")]
    ChainNotServed,
    /// The target purse's product id is not one this host can allocate for.
    #[display("{_0}")]
    Derivation(NftPurseDerivationError),
    /// The purse slot in core storage failed.
    #[display("{_0}")]
    Store(StoreError),
    /// A chain read failed.
    #[display("{_0}")]
    Chain(ChainError),
    /// Metadata or runtime version could not be resolved.
    #[display("{_0}")]
    Context(StatementAllowanceError),
    /// Catch-all.
    #[display("{reason}")]
    Unknown {
        /// Reason.
        reason: String,
    },
}

impl From<NftPurseDerivationError> for Error {
    fn from(err: NftPurseDerivationError) -> Self {
        Self::Derivation(err)
    }
}
impl From<StoreError> for Error {
    fn from(err: StoreError) -> Self {
        Self::Store(err)
    }
}
impl From<ChainError> for Error {
    fn from(err: ChainError) -> Self {
        Self::Chain(err)
    }
}
impl From<StatementAllowanceError> for Error {
    fn from(err: StatementAllowanceError) -> Self {
        Self::Context(err)
    }
}

impl Error {
    /// The service's view of this failure.
    pub(crate) fn to_service_error(&self) -> NftPurseError {
        match self {
            Self::ChainNotServed => NftPurseError::ChainNotServed,
            Self::Derivation(_) => NftPurseError::UnknownTarget,
            other => NftPurseError::Unknown {
                reason: other.to_string(),
            },
        }
    }
}

/// One item found in a purse, with the index it sits at.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct HeldItem {
    /// Derivation index of the holding key.
    pub index: u32,
    /// The holding key.
    pub address: [u8; 32],
    /// The item.
    pub nft: Nft,
}

/// An Asset Hub connection with its metadata and signed-extension state.
pub(crate) struct AssetHub {
    /// Raw RPC client.
    pub rpc: RpcClient,
    /// Metadata and chain state.
    pub context: ChainContext,
}

/// The purse engine shared by every product runtime of one signing host.
pub(crate) struct NftPurses {
    services: Arc<RuntimeServices>,
    store: Arc<NftPurseStore>,
}

impl NftPurses {
    /// Build the engine over the host's shared runtime services.
    pub(crate) fn new(services: Arc<RuntimeServices>) -> Self {
        let store = NftPurseStore::new(services.platform.clone());
        Self { services, store }
    }

    /// The durable allocation and transfer records.
    pub(crate) fn store(&self) -> &NftPurseStore {
        &self.store
    }

    /// The Asset Hub the host serves, with its metadata revalidated.
    pub(crate) async fn asset_hub(&self) -> Result<AssetHub, Error> {
        let genesis = self
            .services
            .asset_hub_chain_genesis_hash()
            .ok_or(Error::ChainNotServed)?;
        let host_rpc = self
            .services
            .chain
            .rpc_client("nft purse", &genesis)
            .await
            .map_err(|err| Error::Unknown {
                reason: format!("Asset Hub connection: {err}"),
            })?;
        let rpc = RpcClient::new(subxt_rpcs::RpcClient::new(host_rpc));
        let client = ChainClient::new(rpc.clone(), genesis);
        let context = self.services.chain_context.get(&client).await?;
        Ok(AssetHub { rpc, context })
    }

    /// The items `product_id`'s purse holds, in derivation order, read from the
    /// chain after settling the transfer log. Allocation bookkeeping catches up
    /// with anything the scan finds, so a wallet restored from seed rebuilds
    /// its purses by listing them.
    pub(crate) async fn scan_purse(
        &self,
        entropy: &[u8],
        root: [u8; 32],
        product_id: &str,
    ) -> Result<Vec<HeldItem>, Error> {
        let hub = self.asset_hub().await?;
        transfer::recover(self, &hub, root)
            .await
            .map_err(|err| Error::Unknown {
                reason: format!("transfer log recovery: {err}"),
            })?;
        self.scan_purse_with(&hub, entropy, root, product_id).await
    }

    /// [`Self::scan_purse`] against an already resolved Asset Hub, without the
    /// recovery sweep.
    pub(crate) async fn scan_purse_with(
        &self,
        hub: &AssetHub,
        entropy: &[u8],
        root: [u8; 32],
        product_id: &str,
    ) -> Result<Vec<HeldItem>, Error> {
        let product_id = normalize_purse_product_id(product_id)?;
        let known = self
            .store
            .purse(root, &product_id)
            .await?
            .map_or(0, |purse| purse.next_index);
        let mut held = Vec::new();
        let mut occupied = Vec::new();
        // Everything the host allocated, then one empty batch beyond it.
        let mut from = 0u32;
        while from < SCAN_MAX {
            let to = (from + SCAN_BATCH).min(SCAN_MAX);
            let batch: Vec<[u8; 32]> = (from..to)
                .map(|index| derive_purse_public_key(entropy, &product_id, index))
                .collect::<Result<_, _>>()?;
            let found = chain::read_nfts(&hub.rpc, &hub.context.metadata, &batch).await?;
            let mut any = false;
            for ((index, address), nft) in (from..to).zip(batch).zip(found) {
                if let Some(nft) = nft {
                    any = true;
                    occupied.push(index);
                    held.push(HeldItem {
                        index,
                        address,
                        nft,
                    });
                }
            }
            from = to;
            if from >= known && !any {
                break;
            }
        }
        self.store
            .observe_occupied(root, &product_id, &occupied)
            .await?;
        Ok(held)
    }

    /// The `NftPurse` service view of `product_id`'s purse, optionally
    /// filtered to `collections`, with each item's transferability read from
    /// its definition.
    pub(crate) async fn list(
        &self,
        entropy: &[u8],
        root: [u8; 32],
        product_id: &str,
        collections: Option<&[u32]>,
    ) -> Result<Vec<NftPurseItem>, Error> {
        let hub = self.asset_hub().await?;
        transfer::recover(self, &hub, root)
            .await
            .map_err(|err| Error::Unknown {
                reason: format!("transfer log recovery: {err}"),
            })?;
        let held = self
            .scan_purse_with(&hub, entropy, root, product_id)
            .await?;
        let held: Vec<HeldItem> = held
            .into_iter()
            .filter(|item| {
                collections.is_none_or(|collections| collections.contains(&item.nft.collection))
            })
            .collect();
        let mut transferability: HashMap<(u32, u32), NftPurseTransferability> = HashMap::new();
        let mut items = Vec::with_capacity(held.len());
        for item in held {
            let key = (item.nft.collection, item.nft.item);
            let kind = match transferability.get(&key) {
                Some(kind) => *kind,
                None => {
                    let kind = match chain::read_transferability(
                        &hub.rpc,
                        &hub.context.metadata,
                        key.0,
                        key.1,
                    )
                    .await?
                    {
                        Some(Transferability::Transferable) => {
                            NftPurseTransferability::Transferable
                        }
                        // A missing definition is broken state the pallet itself
                        // refuses to transfer; report it as bound rather than
                        // inviting a move that cannot succeed.
                        Some(Transferability::Soulbound) | None => {
                            NftPurseTransferability::Soulbound
                        }
                    };
                    transferability.insert(key, kind);
                    kind
                }
            };
            items.push(NftPurseItem {
                instance: item.nft.instance,
                collection: item.nft.collection,
                item: item.nft.item,
                address: item.address,
                state_nonce: item.nft.state_nonce,
                minted_at: item.nft.minted_at,
                last_moved: item.nft.last_moved,
                transferability: kind,
            });
        }
        Ok(items)
    }

    /// A fresh, empty key in `target_product_id`'s purse for `requested_by`,
    /// or the same key again for a repeated `idempotency_key`. The first
    /// allocation in a purse the store has never seen scans it first, so a
    /// wallet restored from seed resumes above its occupied keys.
    pub(crate) async fn request_receive_address(
        &self,
        entropy: &[u8],
        root: [u8; 32],
        target_product_id: &str,
        requested_by: &str,
        idempotency_key: &str,
    ) -> Result<[u8; 32], Error> {
        let target = normalize_purse_product_id(target_product_id)?;
        if self.store.purse(root, &target).await?.is_none() {
            self.scan_purse(entropy, root, &target).await?;
        }
        let index = self
            .store
            .allocate(root, &target, requested_by, idempotency_key)
            .await?;
        Ok(derive_purse_public_key(entropy, &target, index)?)
    }

    /// The consent sheet for moving `instance` out of `product_id`'s purse to
    /// `to`, built from what the chain holds now.
    pub(crate) async fn transfer_review(
        &self,
        entropy: &[u8],
        root: [u8; 32],
        product_id: &str,
        instance: u64,
        to: [u8; 32],
    ) -> Result<NftPurseTransferReview, TransferError> {
        let product_id = normalize_purse_product_id(product_id).map_err(Error::from)?;
        let held = self.scan_purse(entropy, root, &product_id).await?;
        let item = held
            .into_iter()
            .find(|item| item.nft.instance == instance)
            .ok_or(TransferError::NotHeld { instance })?;
        let to_product_id = self.purse_of(entropy, root, &to).await?;
        Ok(NftPurseTransferReview {
            product_id,
            instance,
            collection: item.nft.collection,
            item: item.nft.item,
            to,
            to_product_id,
        })
    }

    /// Move `instance` out of `from_product_id`'s purse to `to`, reporting
    /// progress, and return the including block hash once ownership is
    /// verified there.
    pub(crate) async fn transfer(
        &self,
        entropy: &[u8],
        root: [u8; 32],
        from_product_id: &str,
        instance: u64,
        to: [u8; 32],
        progress: &(dyn Fn(NftPurseTransferStatus) + Send + Sync),
    ) -> Result<[u8; 32], TransferError> {
        let hub = self.asset_hub().await?;
        transfer::recover(self, &hub, root).await?;
        transfer::execute(
            self,
            &hub,
            entropy,
            root,
            transfer::TransferSpec {
                from_product_id,
                instance,
                to,
            },
            progress,
        )
        .await
    }

    /// The purse a key belongs to, if the host allocated it: searched over
    /// every key allocated so far in every known purse.
    pub(crate) async fn purse_of(
        &self,
        entropy: &[u8],
        root: [u8; 32],
        address: &[u8; 32],
    ) -> Result<Option<String>, Error> {
        for purse in self.store.snapshot(root).await?.purses {
            for index in 0..purse.next_index {
                if derive_purse_public_key(entropy, &purse.product_id, index)? == *address {
                    return Ok(Some(purse.product_id));
                }
            }
        }
        Ok(None)
    }

    /// Product ids of every purse the host has allocated in, for the wallet's
    /// own view.
    pub(crate) async fn known_purses(&self, root: [u8; 32]) -> Result<Vec<String>, Error> {
        Ok(self
            .store
            .snapshot(root)
            .await?
            .purses
            .into_iter()
            .map(|purse| purse.product_id)
            .collect())
    }
}

#[cfg(test)]
pub(crate) mod tests {
    use std::sync::Arc;

    use parity_scale_codec::Encode;
    use serde_json::json;

    use super::*;
    use crate::runtime::statement_allowance::extension::{ChainState, Metadata};
    use crate::runtime::statement_allowance::rpc::testing::ScriptedRpc;
    use crate::test_support::StubPlatform;

    pub(crate) const ENTROPY: [u8; 16] = [0xAB; 16];
    pub(crate) const ROOT: [u8; 32] = [0x77; 32];
    const PRODUCT: &str = "cardclash.dot";

    pub(crate) fn purses(platform: Arc<StubPlatform>) -> NftPurses {
        let services = RuntimeServices::new(
            platform,
            truapi_platform::HostInfo {
                name: "test".to_string(),
                icon: None,
                version: None,
                platform: truapi::latest::HostPlatform::Unknown,
            },
            [0; 32],
            [0xbb; 32],
            [0xcc; 32],
            crate::test_support::test_spawner(),
        );
        NftPurses::new(services)
    }

    fn hub(rpc: ScriptedRpc) -> AssetHub {
        let metadata = Metadata::decode(include_bytes!(
            "../../../tests/fixtures/paseo-next-asset-hub-metadata.scale"
        ))
        .unwrap();
        AssetHub {
            rpc: RpcClient::new(subxt_rpcs::RpcClient::new(rpc)),
            context: ChainContext {
                metadata: Arc::new(metadata),
                state: ChainState {
                    spec_version: 1,
                    transaction_version: 1,
                    genesis_hash: [0; 32],
                    nonce: 0,
                    restrict_origins: false,
                },
            },
        }
    }

    fn nft_hex(instance: u64, collection: u32, item: u32, nonce: u64) -> String {
        let mut bytes = Vec::new();
        bytes.extend(instance.encode());
        bytes.extend(collection.encode());
        bytes.extend(item.encode());
        bytes.extend(1_000u64.encode());
        bytes.extend(2_000u64.encode());
        bytes.extend(nonce.encode());
        format!("0x{}", hex::encode(bytes))
    }

    fn storage_key_hex(index: u32) -> String {
        let owner = derive_purse_public_key(&ENTROPY, PRODUCT, index).unwrap();
        format!("0x{}", hex::encode(chain::nfts_by_owner_key(&owner)))
    }

    /// The first batch holds items at indices 1 and 3, the second is empty,
    /// so the scan stops after two round trips, reports the two items in
    /// derivation order, and moves the allocation counter above them.
    #[test]
    fn scan_walks_batches_until_an_empty_trailing_batch() {
        futures::executor::block_on(async {
            let engine = purses(Arc::new(StubPlatform::default()));
            let first = json!([{ "block": "0x01", "changes": [
                [storage_key_hex(1), nft_hex(34, 7, 2, 0)],
                [storage_key_hex(3), nft_hex(35, 7, 5, 4)],
                [storage_key_hex(0), null],
            ]}]);
            let second = json!([{ "block": "0x01", "changes": [] }]);
            let rpc = ScriptedRpc::new([first.to_string().as_str(), second.to_string().as_str()]);
            let hub = hub(rpc.clone());

            let held = engine
                .scan_purse_with(&hub, &ENTROPY, ROOT, PRODUCT)
                .await
                .unwrap();
            assert_eq!(
                held.iter()
                    .map(|item| (item.index, item.nft.instance))
                    .collect::<Vec<_>>(),
                vec![(1, 34), (3, 35)]
            );
            assert_eq!(held[1].nft.state_nonce, 4);
            assert_eq!(
                held[0].address,
                derive_purse_public_key(&ENTROPY, PRODUCT, 1).unwrap()
            );
            let calls = rpc.calls();
            assert_eq!(
                calls.len(),
                2,
                "one batch with items, one empty trailing batch"
            );
            assert!(
                calls
                    .iter()
                    .all(|(method, _)| method == "state_queryStorageAt")
            );
            let purse = engine.store().purse(ROOT, PRODUCT).await.unwrap().unwrap();
            assert_eq!(
                purse.next_index, 4,
                "allocation resumes above the highest occupied key"
            );
        });
    }

    /// A purse the host already allocated deep into reads all of it before the
    /// gap-limit window applies, so an early empty batch does not end the scan.
    #[test]
    fn scan_reads_every_allocated_key_before_applying_the_gap_limit() {
        futures::executor::block_on(async {
            let engine = purses(Arc::new(StubPlatform::default()));
            for i in 0..25 {
                engine
                    .store()
                    .allocate(ROOT, PRODUCT, "console.dot", &format!("k{i}"))
                    .await
                    .unwrap();
            }
            let empty = json!([{ "block": "0x01", "changes": [] }]).to_string();
            let with_item = json!([{ "block": "0x01", "changes": [
                [storage_key_hex(22), nft_hex(9, 1, 1, 0)],
            ]}])
            .to_string();
            // 0..10 empty, 10..20 empty, 20..30 has index 22 (past `known` = 25,
            // so it counts as the window), 30..40 empty ends it.
            let rpc = ScriptedRpc::new([
                empty.as_str(),
                empty.as_str(),
                with_item.as_str(),
                empty.as_str(),
            ]);
            let hub = hub(rpc.clone());
            let held = engine
                .scan_purse_with(&hub, &ENTROPY, ROOT, PRODUCT)
                .await
                .unwrap();
            assert_eq!(held.len(), 1);
            assert_eq!(held[0].index, 22);
            assert_eq!(rpc.calls().len(), 4);
            let purse = engine.store().purse(ROOT, PRODUCT).await.unwrap().unwrap();
            assert_eq!(purse.next_index, 25, "the counter never moves backwards");
            assert_eq!(
                purse.reserved.len(),
                24,
                "the occupied reservation was released"
            );
        });
    }

    /// The purse lookup derives each allocated key and finds one by its index,
    /// with no chain access.
    #[test]
    fn purse_of_searches_allocated_keys() {
        futures::executor::block_on(async {
            let engine = purses(Arc::new(StubPlatform::default()));
            for i in 0..3 {
                engine
                    .store()
                    .allocate(ROOT, PRODUCT, "console.dot", &format!("k{i}"))
                    .await
                    .unwrap();
            }
            let second = derive_purse_public_key(&ENTROPY, PRODUCT, 2).unwrap();
            assert_eq!(
                engine.purse_of(&ENTROPY, ROOT, &second).await.unwrap(),
                Some(PRODUCT.to_string())
            );
            let unallocated = derive_purse_public_key(&ENTROPY, PRODUCT, 3).unwrap();
            assert_eq!(
                engine.purse_of(&ENTROPY, ROOT, &unallocated).await.unwrap(),
                None
            );
        });
    }
}
