    ...
    P: Permissions + Send + Sync + 'static,
{
    ...
    {
        let host = host.clone();
        dispatcher.on_request(wire_table::PERMISSIONS_AUTHORIZE_REMOTE_PERMISSION, move |request_id: String, bytes: Vec<u8>| {
            let host = host.clone();
            Box::pin(async move {
                let request: versioned::permissions::RemotePermissionRequest = match DecodeAll::decode_all(&mut &bytes[..]) {
                    Ok(request) => request,
    ...
    {
        let host = host;
        dispatcher.on_request(wire_table::PERMISSIONS_AUTHORIZE_DEVICE_PERMISSION, move |request_id: String, bytes: Vec<u8>| {
            let host = host.clone();
            Box::pin(async move {
                let request: versioned::permissions::HostDevicePermissionRequest = match DecodeAll::decode_all(&mut &bytes[..]) {
                    Ok(request) => request,
