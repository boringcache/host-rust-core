//! NFT purse types for the `NftPurse` service.
//!
//! The host keeps one purse of `pallet-scarcity` NFTs per product plus the
//! wallet's own. The pallet holds one NFT per account key (`NftsByOwner`), so
//! every item sits in its own host-derived purse key. Custody is context: an
//! item belongs to whichever product's purse holds it. Products see items,
...
pub enum NftPurseTransferability {
...
/// One live NFT held in a purse, as read from `Scarcity.NftsByOwner`.
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode)]
#[cfg_attr(feature = "uniffi", derive(uniffi::Record))]
pub struct NftPurseItem {
    ...
    pub transferability: NftPurseTransferability,
}

/// Request to list the NFTs in the caller's purse.
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode)]
pub struct HostNftPurseListRequest {
...
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode)]
pub struct HostNftPurseListResponse {
    /// Live items, in purse derivation order.
    pub items: Vec<NftPurseItem>,
}

/// Request to follow the NFTs in the caller's purse.
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode)]
pub struct HostNftPurseListSubscribeRequest {
...
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode)]
pub struct HostNftPurseListSubscribeItem {
    /// Live items, in purse derivation order.
    pub items: Vec<NftPurseItem>,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode)]
pub struct HostNftPurseRequestReceiveAddressRequest {
...
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode)]
pub struct HostNftPurseRequestReceiveAddressResponse {
    /// The empty purse key.
    pub address: AccountId,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode)]
pub struct HostNftPurseTransferRequest {
    /// Instance to move; the caller's purse must hold it.
    pub instance: u64,
...
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode)]
pub enum NftPurseTransferStatus {
...
/// Failures from the `NftPurse` service.
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode)]
pub enum NftPurseError {
    /// The user or host refused the request.
    Rejected,
    /// The caller's purse does not hold the named instance.
    NotFound,
    /// The destination purse key already holds an NFT.
    AddressOccupied,
    /// The holding purse key is locked after a failed dispatch; retry after `until`.
    Locked {
