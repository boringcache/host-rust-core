...
Product scripts run in sandboxed Chromium and use the shared container and Rust permission policy. They require Bun on `PATH` and the matching browser, installed with `truapi-host install-browser`.
...
The [permission model](docs/rfcs/0002-permission-model.md) separates outbound domain access from `OpenUrl` external navigation and requires `Notifications` for push delivery. Hosts preserve the user's `AllowOnce`, `AllowAlways`, or `Deny` choice.
