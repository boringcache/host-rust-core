...
rust/crates/
  ...
  truapi-host-cli/       CLI pairing/signing hosts; js/ bundles the shared browser container and launches product scripts
js/packages/
  ...
  truapi-debugger/        @parity/truapi-debugger (published to npm): the debugger.
                          ...
                          an open decision.
js/container/              Shared TS lockdown container for native web views and CLI dev/scripts; `npm run build`
...
browser assets generated by `make headless` or `make cli-runner`. On Unix, the
