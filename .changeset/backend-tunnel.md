...
credential, performs the call, and returns the status, an allowlisted set of headers and the body. A backend
that authenticates a person is authenticated by the core, which holds the session and passes it to the host
beside the request: the host sends it as `Authorization: Bearer` and its own credential as
`X-Polkadot-Host-Authorization`, so the two never displace each other. A product holds no credential for a
backend and has no field to put one in. The core
